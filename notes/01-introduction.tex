...
\noindent {\bfseries Exercise 1}. Consider the maximum and minimum numbers
derived in (\ref{maxnumber})-(\ref{minnumber}). How many digits should we
include in each of these numbers?\\
\\
\noindent {\bfseries Exercise 2}. Find the binary floating point representation
of the decimal number 4.25 in single precision.\\
\\
\noindent {\bfseries Exercise 3}. How many digits of accuracy does a floating
...
\noindent {\bfseries Exercise 4}. Propose a way to avoid the above limitation.
...
\noindent {\bfseries Exercise 5}. Let $c$ be a scalar (a floating point number),
...
\noindent {\bfseries Exercise 6}. Let $\underline{A}$ be an $n\times n$ matrix,
...
Figure \ref{fig:perform_dev} and Figure \ref{fig:proj_perform_dev} depict the
...
In 1986 a supercomputing center was established at NTH. Table
